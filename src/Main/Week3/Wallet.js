...
export default function Wallet({setShowWallet, showWallet, transparent, setMenuOpen, setSplashMenuOpen}) {
  ...
  if (!window.walletConnection.isSignedIn()) {
    return (
      <>
        <div className="controls controlsLast">
          <button onClick={login}  className="mainWalletBadge Week3Wallet InterButton">Connect Wallet</button>
        </div>
      </>
    );
  } else {
    return (
      <>
        <div className="controls controlsLast">
          <button className={transparent ? "mainWalletBadge Week3Wallet mainWalletBadgeTransparent InterButton" : "mainWalletBadge Week3Wallet InterButton"}
